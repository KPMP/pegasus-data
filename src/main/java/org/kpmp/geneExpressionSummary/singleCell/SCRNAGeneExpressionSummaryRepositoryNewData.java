...
@Repository
public interface SCRNAGeneExpressionSummaryRepositoryNewData
		extends CrudRepository<SCRNAGeneExpressionExpressionSummaryValueNewData, GeneExpressionId> {
	...
	List<SCRNAGeneExpressionExpressionSummaryValueNewData> findByEnrollmentAndGeneAllClusters(
	...
	@Query(value = "SELECT DISTINCT scr.cluster, c.cluster_name, 0 as cell_count, scr.id, scr.enrollment_category, scr.gene, scr.p_val as p_val, scr.p_val_adj as p_val_adj, scr.fold_change, scr.pct_1, scr.pct_2, scr.avg_exp as avg_exp "
			+ "FROM sc_rnaseq_2025 scr " + "JOIN cluster c ON scr.cluster = c.abbreviation AND c.cluster_name = :cellType "
			+ "WHERE scr.enrollment_category = LCASE(:enrollmentCategory) " + "ORDER BY scr.fold_change DESC", nativeQuery = true)
	List<SCRNAGeneExpressionExpressionSummaryValueNewData> findExpressionSummaryPerGeneByCellTypeAndEnrollmentCategory(
