...
public interface SCRNAGeneExpressionSummaryRepository extends CrudRepository<SCRNAGeneExpressionExpressionSummaryValue, GeneExpressionId> {
    ...
    @Query(value = "SELECT DISTINCT scc.cluster, scc.cluster_name, IF(isnull(scr.gene), null, scc.cell_count) as cell_count, scc.cluster_id as id, scr.tissue_type, IF(isnull(scr.gene), :geneSymbol, scr.gene) as gene, scr.p_val as p_val, scr.p_val_adj as p_val_adj, scr.fold_change, scr.pct_1, scr.pct_2, scr.specificity, scr.avg_exp as avg_exp " +
