...
public interface SNRNAGeneExpressionSummaryRepository extends CrudRepository<SNRNAGeneExpressionExpressionSummaryValue, GeneExpressionId> {
	...
	@Query(value = "SELECT DISTINCT snc.cluster, snc.cluster_name, IF(isnull(snr.gene), null, snc.cell_count) as cell_count, snc.cluster_id as id, snr.tissue_type, IF(isnull(snr.gene), :geneSymbol, snr.gene) as gene, snr.p_val as pval, snr.p_val_adj as pval_adj, snr.fold_change, snr.pct_1 as pct1, snr.pct_2 as pct2, snr.avg_exp as avg_exp " +
