...
public interface CellTypeRepository extends CrudRepository<CellType, Integer> {
	...
	@Query(value = "SELECT ct.*, cs.* " + "    FROM cell_type ct "
			...
			+ "    WHERE ct.cell_type IS NOT NULL AND ct.cell_type LIKE %:searchTerm% OR ct.structure_region LIKE %:searchTerm% OR ct.structure_subregion LIKE %:searchTerm%", nativeQuery = true)
