...
public interface CellTypeRepository extends CrudRepository<CellType, Integer> {
	...
	@Query(value = "SELECT ct.cell_type_id, ct.structure_region, ct.structure_subregion, ct.cell_type, ct.release_ver, ct.release_sunset, ct.cell_type_order, cs.cell_type_synonym " + "    FROM cell_type_2025 ct "
			+ "    JOIN celltype_synonym_2025 cs ON (ct.cell_type_id = cs.cell_type_id AND cs.cell_type_synonym LIKE %:searchTerm%) "
			+ "    UNION " + "    SELECT ct.cell_type_id, ct.structure_region, ct.structure_subregion, ct.cell_type, ct.release_ver, ct.release_sunset, ct.cell_type_order, cs.cell_type_synonym " + "    FROM cell_type_2025 ct "
