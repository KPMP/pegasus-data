...
interface CellTypeClusterHierarchyRepository extends CrudRepository<CellTypeClusterHierarchy, Integer> {
    ...
    @Query(value = "SELECT ch.* " +
            ...
            "AND (ch.is_single_nuc = 'Y' OR ch.is_single_cell = 'Y' OR ch.is_rt = 'Y' OR ch.is_rp = 'Y' OR ch.is_st = 'Y') " +
            "UNION "+
            "SELECT ch2.* " +
            "FROM cell_type_cluster_hierarchy ch " +
            ...
            "(ch2.is_single_nuc = 'Y' OR ch2.is_single_cell = 'Y' OR ch2.is_rt = 'Y' OR ch2.is_rp = 'Y' OR ch2.is_st = 'Y') " +
            "UNION "+
            "SELECT ch2.* " +
            "FROM cell_type_cluster_hierarchy ch " +
            ...
            "(ch2.is_single_nuc = 'Y' OR ch2.is_single_cell = 'Y' OR ch2.is_rt = 'Y' OR ch2.is_rp = 'Y' OR ch.is_st = 'Y')", nativeQuery = true)
