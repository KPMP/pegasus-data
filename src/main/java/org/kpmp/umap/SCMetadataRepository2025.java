...
public interface SCMetadataRepository2025 extends CrudRepository<SCMetadata2025, String> {
	...
	@Query(value = "SELECT "
			...
			+ "group by cluster_abbreviation having count(cluster_abbreviation) < 1500) "
			+ "UNION ALL "
			+ "(SELECT "
			+ "umap_x, umap_y, cluster_abbreviation, cluster_name, cluster_color, barcode, enrollment_category "
					+ "FROM sc_umap_point_2025_v "
					+ "WHERE cluster_abbreviation in ( select cluster_abbreviation from sc_umap_point_2025_v "
					+ "group by cluster_abbreviation having count(cluster_abbreviation) BETWEEN 1500 AND 5000 ) "
